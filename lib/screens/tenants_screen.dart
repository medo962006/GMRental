...
class _TenantsScreenState extends ConsumerState<TenantsScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: TabBarView(
        ...
        children: [
          // ── TAB 1: TENANTS ─────────────────────────────────
          _buildTenantsTab(context, ref, tenantsAsync, isDesktop),
  ...
  Widget _buildTenantsTab(BuildContext context, WidgetRef ref, AsyncValue<List<Tenant>> tenantsAsync, bool isDesktop) {
    final buildingId = ref.watch(currentBuildingIdProvider);
    return Column(
      children: [
        // ── Filter Chips ──────────────────────────────
  ...
  Widget _buildCarsTab(BuildContext context, WidgetRef ref, AsyncValue<List<Tenant>> tenantsAsync, bool isDesktop) {
    return Column(
      children: [
        // Search Bar
        Padding(
  ...
  void initState() {
    ...
    _insuranceController = TextEditingController(
        text: widget.tenant != null && widget.tenant!.insuranceAmount > 0
            ...
            : '');
    _carModelController =
        TextEditingController(text: widget.tenant?.carModel ?? '');
    _licensePlateController =
        TextEditingController(text: widget.tenant?.licensePlate ?? '');
    ...
    _hasCar = widget.tenant?.hasCar ?? false;
  }
              ...
              children: [
                ...
                Consumer(
                  builder: (context, ref, _) {
                    final currentBuildingId = ref.watch(currentBuildingIdProvider);
                    final roomsAsync = ref.watch(roomsStreamProvider(currentBuildingId));
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Has Car Switch
                        SwitchListTile(
                          title: const Text('Has Car'),
                          subtitle: const Text('Toggle if tenant has a car'),
                          value: _hasCar,
                          onChanged: (val) {
                            setState(() => _hasCar = val);
                          },
                          secondary: Icon(_hasCar ? Icons.directions_car : Icons.directions_car_outlined,
                              color: _hasCar ? Colors.green : Colors.grey),
                        ),
                        if (_hasCar) ...[
                          const SizedBox(height: 8),
                          // Car Model
          ...
          onPressed: () {
            if (_formKey.currentState!.validate()) {
              ...
              final tenant = Tenant(
                ...
                hasCar: _hasCar,
                carModel: _hasCar && _carModelController.text.trim().isNotEmpty
